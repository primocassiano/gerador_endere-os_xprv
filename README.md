...
3.  **Gera 4 Tipos de Endereços:** Para cada chave derivada (em sequência), o programa gera quatro tipos diferentes de endereços Bitcoin:
    *   **Legacy (P2PKH):** Endereços tradicionais que começam com `1...`. Usa o caminho de derivação `m/44'/0'/0'/0/i` (BIP44).
    *   **Nested SegWit (P2SH-P2WPKH):** Endereços SegWit "embrulhados" para compatibilidade, que começam com `3...`. Usa o caminho de derivação `m/49'/0'/0'/0/i` (BIP49).
    *   **Native SegWit (P2WPKH):** Endereços SegWit nativos, mais eficientes, que começam com `bc1q...`. Usa o caminho de derivação `m/84'/0'/0'/0/i` (BIP84).
    *   **Taproot (P2TR):** O tipo de endereço mais recente, com foco em privacidade e eficiência, que começa com `bc1p...`. Usa o caminho de derivação `m/86'/0'/0'/0/i` (BIP86).
4.  **Mostra em Lotes:** O programa exibe os endereços em lotes de 20 (índices 0-19, depois 20-39, e assim por diante) para cada um dos quatro tipos.
...
5.  **Veja os Endereços:** O programa mostrará os primeiros 20 endereços de cada tipo.
