...
// Derivation paths constants (assuming mainnet, account 0, external chain 0)
const (
	...
	// Corrected: Added newline delimiter '\n'
	 ...
	 for {
		 ...
		 for i := currentBatchStart; i < currentBatchStart+AddressBatchSize; i++ {
			 legacyKey, err := deriveChildKey(masterKey, BIP44Purpose, CoinTypeBitcoin, DefaultAccount, ExternalChain, i)
			 if err != nil {
		 ...
		 for i := currentBatchStart; i < currentBatchStart+AddressBatchSize; i++ {
			 nestedKey, err := deriveChildKey(masterKey, BIP49Purpose, CoinTypeBitcoin, DefaultAccount, ExternalChain, i)
			 if err != nil {
		 ...
		 for i := currentBatchStart; i < currentBatchStart+AddressBatchSize; i++ {
			 nativeKey, err := deriveChildKey(masterKey, BIP84Purpose, CoinTypeBitcoin, DefaultAccount, ExternalChain, i)
			 if err != nil {
		 ...
		 fmt.Println("\nTaproot (BIP86 - P2TR):")
		 for i := currentBatchStart; i < currentBatchStart+AddressBatchSize; i++ {
			 taprootKey, err := deriveChildKey(masterKey, BIP86Purpose, CoinTypeBitcoin, DefaultAccount, ExternalChain, i)
			 if err != nil {
